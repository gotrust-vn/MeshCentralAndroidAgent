...
class MeshAccessibilityService : AccessibilityService() {
    ...
    private fun dispatchLongPress(x: Float, y: Float) {
        ...
        val path = Path().apply { moveTo(x, y) }
        // 600ms: enough to exceed Android's default long-press threshold (500ms)
        val stroke = GestureDescription.StrokeDescription(path, 0, 600)
        val cb = object : GestureResultCallback() {
            override fun onCancelled(gestureDescription: GestureDescription?) {
                println("dispatchLongPress: cancelled → ACTION_LONG_CLICK at ($x,$y)")
    ...
    private fun performNodeLongClick(x: Int, y: Int) {
        ...
        if (allWindows != null) {
            for (window in allWindows) {
                val root = try { window.root } catch (e: Exception) { null } ?: continue
                val node = findDeepestClickableNode(root, x, y, requireClickable = true) ?: continue
                if (node.performAction(AccessibilityNodeInfo.ACTION_LONG_CLICK)) {
                    println("performNodeLongClick: ok at ($x,$y) cls=${node.className}")
                    return
                }
            }
        }
        val root = rootInActiveWindow ?: return
        val node = findDeepestClickableNode(root, x, y, requireClickable = true) ?: return
